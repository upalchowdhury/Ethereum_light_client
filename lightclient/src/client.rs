...
pub struct Client<DB: Database> { 
    // DB can be of anytype as long as implements Database trait. Its called trait bound
...
pub fn notify<T: Summary>(item1: &T, item2: &T) {
    ...
    https://doc.rust-lang.org/book/ch10-02-traits.html#clearer-trait-bounds-with-where-clausesafj;sldk qoijfdfjkjf
...
impl<DB: Database> Client<DB> { 
    ...
    async fn boot_from_fallback(&self) -> eyre::Result<()> {
        if let Some(fallback) = &self.fallback {
            ...
            // mWe fail fast here since the node is unrecoverable at this point
